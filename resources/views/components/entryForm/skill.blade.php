...
<section>
    <p class="toggleSwitchApply text-yellow {{$isOpen ? 'open' : 'close'}}">スキルシート <span class="btn-toggle"></span></p>
    <table class="body {{$isOpen ? 'open' : 'close'}}">
        ...
        @foreach($items As $groupName => $list)
            ...
            <tr>
                ...
                @switch($fieldName)
                    @case('qualification')
                    <textarea name="qualification" id="qualification" placeholder="例）普通免許(AT、MT、第一種、第二種)/準中型免許/中型免許(第一種、第二種)/大型免許(第一種、第二種)/ フォークリフト">{{old('qualification', $fEntUserApplyInfo->sikak ?? '')}}</textarea>
                    <small class="note-textarea">最大1500文字まで</small>
