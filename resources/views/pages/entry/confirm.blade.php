...
@section('content')
    ...
    /* @var $page */
    $path = str_replace('_confirm', '', $page->id);
    if(isset($page->fEntConfig->backendSettings['form'][$path]) && count($page->fEntConfig->backendSettings['form'][$path])>0) {

        foreach($page->fEntConfig->backendSettings['form'][$path] As $sectionName => $items) {
        ...
        //カスタム項目の格納処理
        if(isset($page->fEntConfig->backendSettings['form'][$path]['custom']) && count($page->fEntConfig->backendSettings['form'][$path]['custom'])>0) {
            foreach($page->fEntConfig->backendSettings['form'][$path]['custom'] As $sectionName => $items) {
                foreach($items As $groupName => $list) {
                    foreach($list As $fieldName => $rules) {
                ...
                <div class="inner">
                    ...
                    <section class="mod_jobDetailJob">
                        <section class="mod_apply_confirm">
                            <form id="{{$path}}ConfirmForm" name="fm" method="post" action="{{$page->action ?? '#'}}">
                                @csrf
